...
```
```{r}
admissions %>% mutate(success=round(admitted/applicants*100,2)) %>%
  pivot_wider(c(-admitted,-applicants),
    names_from=major, values_from=success
  )
...
heights %>% 
  ...
  summarize(
    ...
    more_than_70 = nTall(height)
    )

nTall <- function(x){
  return (sum(x>=70))
}
nTall(heights$height)
```
...
ref_avg <- NHANES %>%
  ...
  summarise(ref = mean(BPSysAve, na.rm = TRUE)) 
