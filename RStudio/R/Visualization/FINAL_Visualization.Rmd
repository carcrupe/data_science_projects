...
boxplot(babies$gestation~babies$smoke)
...
height=read.csv("Heights.csv",header=T,sep="\t",stringsAsFactors = F)
head(height)

plot(height$Height,height$Age)
boxplot(height$Height~height$Sex)
boxplot(height$Age)
hist(height$Age, breaks = 10)
hist(height$Height, breaks=100)
...
male <- height %>% 
  filter(Sex == "M") %>%
  summarize(average = mean(Height),median = median(Height), standard_deviation = sd(Height), IQR = IQR(Height))
female <- height %>% 
  filter(Sex == "F") %>%
  summarize(average = mean(Height),median = median(Height), standard_deviation = sd(Height), IQR = IQR(Height))
...
murders %>% ggplot(aes(population/10^6, total, label = abb)) +   
  geom_abline(intercept = log10(r), lty = 1, color = "darkgrey") +
  geom_point(aes(col=region), size = 3) +
  geom_text_repel() + 
  scale_x_log10() +
  scale_y_log10() +
